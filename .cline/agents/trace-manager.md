...
export const test = base.extend({
  ...
  page: async ({ page }, use) => {
    ...
    const tracePath = path.resolve(process.env.TRACE_DIR || 'traces', `${test.info().title}-${Date.now()}.zip`);
    await page.context().tracing.stop({ path: tracePath });
    AllureReporter.addAttachment('Playwright Trace', await fs.promises.readFile(tracePath), 'application/zip');
  },
});
```
...
*File location:* `.cline/agents/trace-manager.md`*